...
function I2C(Zeal, PIO) {
    ...
    function device_write_read(device, wr_fifo, rd_fifo) {
        if (device == I2C_RTC_ADDR) {
            const add = wr_fifo.shift();
    ...
    function device_address_received(dev_addr) {
        ...
        if ((dev_addr ^ device) == 1 && (dev_addr & 1) == 1) {
            device_write_read(device >> 1, wr_fifo, rd_fifo);
            shift_register_out = rd_fifo.shift();
        } else {
            wr_fifo = [];
            rd_fifo = [];
            shift_register = 0;
